-- lux.lua — core framework
-- engine: lua-http (production) or luasocket (windows/fallback)
...
-- ── Engine detection ───────────────────────────────
local _engine = nil
local _http_server, _http_headers

local ok_http = pcall(function()
    _http_server  = require("http.server")
    _http_headers = require("http.headers")
end)

if ok_http then
    _engine = "lua-http"
else
    local ok_socket = pcall(function() require("socket") end)
    if ok_socket then
        _engine = "luasocket"
    else
        error("[lux] no HTTP engine found. install lua-http (recommended) or luasocket (windows)")
    end
end
...
local function print_banner(port)
    ...
    if auth then
        print(c.white .. c.bold .. "  auth   " .. c.reset .. c.gray .. "→  " .. c.reset .. c.white .. "JWT enabled" .. c.reset)
    end
    ...
    print(c.white .. c.bold .. "  engine " .. c.reset .. c.gray .. "→  " .. c.reset .. c.white .. _engine .. c.reset)
    print("")
end
...
local function read_config()
    ...
    config.jwt_secret = content:match('"jwt_secret"%s*:%s*"([^"]+)"')
...
local function setup_db(path)
    ...
    if folder then mkdir(folder) end
    ...
    return setmetatable({}, {
        __index = function(_, path)
            return function(handler)
                table.insert(_routes, { method = method, path = path, handler = handler })
            end
        end
    })
end
...
local function build_swagger(port)
    ...
    for _, route in ipairs(_routes) do
        ...
        for param in route.path:gmatch(":(%w+)") do
            params[#params + 1] = { name = param, ["in"] = "path", required = true, schema = { type = "string" } }
        end
        paths[path][method] = {
...
-- ── Request handler (shared between engines) ───────
local function handle_request(method, full_path, hdrs, body, get_ip)
    ...
    -- rate limiting
    if ratelimit then
        local ip = hdrs["x-forwarded-for"] or hdrs["x-real-ip"] or (get_ip and get_ip()) or "unknown"
        if not ratelimit(ip) then
            return reply(429, "Too many requests, slow down")
        end
    end

    -- cors preflight
    if method == "OPTIONS" then
        return { status = 204, body = "", content_type = "text/plain", cors = true }
    end

    -- swagger
    if _config.swagger then
        if method == "GET" and path == "/docs" then
            return { status = 200, body = swagger_html(_config.port or 3000), content_type = "text/html" }
        end
        if method == "GET" and path == "/docs/spec" then
            return { status = 200, body = build_swagger(_config.port or 3000), content_type = "application/json" }
        end
    end
    ...
    for _, route in ipairs(_routes) do
        if route.method == method then
            ...
            if params then
                ...
                return reply(500, "Error: " .. tostring(res))
            end
        end
    end
    ...
    local srv = _http_server.listen {
        ...
        onstream = function(_, stream)
            ...
            local function get_ip()
                ...
                return ok and peer or "unknown"
            end

            local res = handle_request(method, full_path, hdrs, body, get_ip)

            local res_headers = _http_headers.new()
            ...
            if logger then
                ...
                logger(method, full_path:match("^([^?]*)"), res.status, ms)
            end
        end
    }
    srv:listen()
    if _config.swagger then open_browser("http://localhost:" .. port .. "/docs") end
    srv:loop()
end

-- ── luasocket engine ───────────────────────────────
local function start_socket(port)
    ...
    local function send_response(client, res, with_cors)
        local status_text = {
        ...
        local ctype  = res.content_type or "text/plain"
        local text   = status_text[status] or "OK"

        local response = "HTTP/1.1 " .. status .. " " .. text .. "\r\n"
    ...
    while true do
        ...
        if client then
            client:settimeout(5)
            local start_time = os.clock()
            local ok, method, path, hdrs, body = pcall(parse_http_request, client)
            if ok and method then
                local res = handle_request(method, path, hdrs, body, function()
                    return client:getpeername()
                end)
                send_response(client, res, _config.cors)
                if logger then
                    local ms = math.floor((os.clock() - start_time) * 1000)
                    logger(method, path:match("^([^?]*)"), res.status, ms)
                end
            end
            client:close()
        end
...
if _engine == "lua-http" then
    start_http(port)
else
    warn("using luasocket engine — single threaded, for development only")
    start_socket(port)
end
