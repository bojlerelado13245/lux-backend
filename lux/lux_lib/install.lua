...
local skip_on_windows = {
    ["luaposix"] = true,
    ["http"]     = true,  -- lua-http won't compile on windows, luasocket is used instead
}
...
if OS == "windows" then
    print("")
    print("[lux] windows detected — using luasocket engine (development mode)")
    print("[lux] for production, deploy on Linux or use Docker")
end
...
if #failed == 0 and #skipped == 0 then
    ...
    print("[lux] run: lua lux_lib/lux.lua")
